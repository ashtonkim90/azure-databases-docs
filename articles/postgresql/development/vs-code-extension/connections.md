...
ms.topic: how-to
# customer intent: As a user, I want to connect to PostgreSQL with the right authentication and saved profile options, so that I can work securely across servers.
---
...
- **Connections**: Hover over the **Connections** section header and select **Add New Connection** (the **+** icon).
...
| **Browse Azure** | Browse your Azure subscriptions, resource groups, and servers to select an Azure Database for PostgreSQL flexible server or Azure HorizonDB (Preview) cluster directly. Requires an Azure account signed in to Visual Studio Code. |
...
| **Connection Name** | Optional friendly name for this connection. The extension displays this name in the **Connections** tree and the **Saved Connections** list. |
...
The **Authentication Type** dropdown list offers four options.
...
1. **Select your account.** Choose the account you added from the **Entra Account** dropdown list.
1. **Select a tenant.** If your account belongs to multiple tenants, select the appropriate tenant from the **Tenant** dropdown list.
...
1. **Enter the database role.** In **User name**, enter the PostgreSQL database user or role. This isn't the **AWS Profile**, IAM user, IAM role, or ARN.
...
1. **Set the AWS Region if needed.** The extension infers the region from standard RDS and Aurora hostnames when possible. If the hostname doesn't include the region, enter it in **AWS Region**.
...
The AWS credential chain supports environment variables, shared AWS configuration and credentials files, AWS SSO profiles, and `credential_process` profiles. It doesn't use EC2 or ECS instance/container metadata credentials, and it doesn't use metadata-backed `credential_source` profile chains.
...
After you fill in connection details, use the actions in the connection dialog and the **Connections** tree to save the profile, connect, edit, or remove servers.
...
If the connection succeeds, the server appears in the **Connections** tree. The connection also appears in the **Saved Connections** and **Recent Connections** lists in the connection dialog.

### Test Connection

Select **Test Connection** to verify the connection parameters without saving the profile or adding the server to the **Connections** tree. The button shows a spinner while the test runs, then displays a checkmark on success or a warning icon on failure.

### Disconnect

To disconnect from a server:

- **Connections**: Right-click the server node and select **Disconnect**.
- **Command Palette**: Run **PGSQL: Disconnect**.

## Edit an existing connection

To edit a saved connection, right-click the server in the **Connections** tree and select **Edit Connection**. The connection dialog opens in edit mode with the header **Edit Connection** and the current profile name as the title. Make your changes and select **Save & Connect** to reconnect with the updated settings.
...
The **Copilot** section in the advanced drawer includes a **Copilot access mode** dropdown list with three options:
...
| **Require** | Requires SSL encryption but doesn't verify the server certificate. Protects against passive eavesdropping. |
...
> [!NOTE]  
> Azure Database for PostgreSQL requires SSL by default. Use **Require** for most scenarios or **Verify-Full** with the [DigiCert Global Root G2 certificate](https://knowledge.digicert.com/general-information/digicert-trusted-root-authority-certificates) for maximum security.
...
Server groups help you organize connections in the **Connections** tree. You can assign a custom name and color to each group, making it easier to identify environments such as Development, Staging, and Production at a glance.

### Create a server group

1. Right-click in the **Connections** tree and select **Create Server Group**.
...
- **Edit**: Right-click a server group in the **Connections** tree and select **Edit Server Group** to change its name, description, or color.
- **Remove**: Right-click a server group and select **Remove**. Connections in the group aren't deleted; they move to the default group.

### Assign a connection to a group

When you create or edit a connection, the connection dialog includes a **Server Group** dropdown list in the main fields area. Select a group from the list to assign the connection to that group.

Group colors appear as a colored bar next to the group name in the **Connections** tree, providing quick visual identification.
...
When a connection attempt fails, the cause is usually a network, authentication, or SSL setting. The following sections cover the most common cases.
...
### Connection string parsing doesn't populate the right fields

If a pasted connection string doesn't populate the expected values, review the supported input patterns in [Advanced connection options](advanced-connection-options.md#use-connection-string-input-mode). If the source format is unusual, switch to the **Parameters** tab and enter the values manually.
