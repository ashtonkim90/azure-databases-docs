...
ms.topic: how-to
# customer intent: As a user, I want to configure connection strings, certificates, and SSH tunnels, so that I can connect securely in advanced PostgreSQL environments.
---
...
The **SSL root certificate mode** dropdown list controls how the extension supplies the root CA certificate. Choose one of the following values:

| Mode | Behavior |
| --- | --- |
| **None** | No root certificate is used. The extension doesn't verify the server certificate chain. |
