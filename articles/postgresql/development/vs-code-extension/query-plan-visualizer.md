...
1. Select the statement you want to analyze. If you leave the selection empty, the extension uses the current editor contents.
1. Select **Visualize Query Plan (PostgreSQL)** in the editor toolbar. You can also run the query first and then select **Visualize Query Plan** in the **PostgreSQL Query Results** panel.
1. In **Configure Query Plan**, choose the format and options you want, and then press Enter.
1. Review the plan in the visualizer.
...
1. Select **Analyze with Copilot** (or **Analyze with AI** in Cursor).
...
1. Select **Analyze this node with Copilot** (or **Analyze this node with AI** in Cursor).
1. Review the new Agent mode chat session for node-specific analysis.
