...
| --- | --- | --- |
| [address_standardizer](http://postgis.net/docs/manual-2.5/Address_Standardizer.html) is used to parse an address into constituent elements. Generally used to support geocoding address normalization step. | 3.6.1 | |
| [address_standardizer_data_us](http://postgis.net/docs/manual-2.5/Address_Standardizer.html) is the Address Standardizer US dataset example. | 3.6.1 | |
| [age](https://age.apache.org/) provides graph database capabilities. | 1.7.0 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
...
| [azure_ai](../../flexible-server/generative-ai-azure-overview.md) provides Azure AI and ML Services integration for PostgreSQL. | 2.0.0 | |
| [azure_storage](../../flexible-server/concepts-storage-extension.md) provides Azure Storage integration for PostgreSQL. | 1.9 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
...
| [hll](https://github.com/citusdata/postgresql-hll) introduces a new data type hll which is a HyperLogLog data structure. | 2.21 | |
| [hstore](https://www.postgresql.org/docs/current/hstore.html) is a data type for storing sets of (key, value) pairs. | 1.8 | |
| [hypopg](https://github.com/HypoPG/hypopg) provides hypothetical indexes for PostgreSQL. | 1.4.2 | |
...
| [login_hook](https://github.com/splendiddata/login_hook) is a hook to execute `login_hook.login()` at login time. | 1.7 | |
...
| [pgaudit](https://www.pgaudit.org/) provides auditing functionality. | 18.0 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
...
| [pg_cron](https://github.com/citusdata/pg_cron) is a job scheduler for PostgreSQL. | 1.6 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries.<br />Read the special considerations for extension [pg_cron](../concepts-extensions-considerations.md#pg_cron) in flexible server. |
| [pgcrypto](https://www.postgresql.org/docs/current/pgcrypto.html) provides cryptographic functions. | 1.4 | Read the special considerations for extension [pgcrypto](../concepts-extensions-considerations.md#pgcrypto) in flexible server. |
| [pg_diskann](../../flexible-server/how-to-use-pgdiskann.md) is a scalable approximate nearest neighbor search algorithm for efficient vector search at any scale. | 0.6.5 | |
| [pg_failover_slots](https://github.com/EnterpriseDB/pg_failover_slots) is a logical replication slot manager for failover purposes. | Not supported <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries.<br />Read the special considerations for extension [pg_failover_slots](../concepts-extensions-considerations.md#pg_failover_slots) in flexible server. |
| [pg_freespacemap](https://www.postgresql.org/docs/current/pgfreespacemap.html) examines the free space map (FSM). | 1.3 | |
| [pg_hint_plan](https://github.com/ossc-db/pg_hint_plan) makes it possible to tweak PostgreSQL execution plans using so-called hints in SQL comments. | 1.8.0 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries.<br />Read the special considerations for extension [pg_hint_plan](../concepts-extensions-considerations.md#pg_hint_plan) in flexible server. |
| [pg_ivm](https://github.com/sraoss/pg_ivm) provides Incremental View Maintenance (IVM) feature for PostgreSQL. | 1.13 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
| [pglogical](https://github.com/2ndQuadrant/pglogical) manages PostgreSQL Logical Replication. | 2.4.6 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
| [pg_partman](https://github.com/pgpartman/pg_partman) manages partitioned tables by time or ID. | 5.4.3 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
| [pg_partman_bgw](https://github.com/pgpartman/pg_partman) manages partitioned tables by time or ID. | 5.0.1 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
| [pg_prewarm](https://www.postgresql.org/docs/current/pgprewarm.html) prewarms the cache with relation data. | 1.2 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries.<br />Read the special considerations for extension [pg_prewarm](../concepts-extensions-considerations.md#pg_prewarm) in flexible server. |
| [pg_repack](https://reorg.github.io/pg_repack/) reorganizes tables in PostgreSQL databases with minimal locks. | 1.5.3 | Read the special considerations for extension [pg_repack](../concepts-extensions-considerations.md#pg_repack) in flexible server. |
...
| [pg_squeeze](https://github.com/cybertec-postgresql/pg_squeeze) removes unused space from a relation. | 1.9 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
| [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) tracks execution statistics of all SQL statements executed. | 1.12 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries.<br />Read the special considerations for extension [pg_stat_statements](../concepts-extensions-considerations.md#pg_stat_statements) in flexible server. |
...
| [postgis](https://www.postgis.net/) geometry and geography spatial types and functions. | 3.6.1 | |
| [postgis_raster](https://www.postgis.net) raster types and functions. | 3.6.1 | |
| [postgis_sfcgal](https://www.postgis.net) sFCGAL functions. | 3.6.1 | |
| [postgis_tiger_geocoder](https://www.postgis.net) tiger geocoder and reverse geocoder. | 3.6.1 | |
| [postgis_topology](https://postgis.net/docs/Topology.html) spatial types and functions. | 3.6.1 | |
...
| [rdkit](https://github.com/rdkit/rdkit/blob/master/Code/PgSQL/rdkit/README.md) implements mol datatype to describe molecules and fp datatype for fingerprints, basic comparison operations, similarity operation, and index support. | 4.8.0 | |
| [semver](https://pgxn.org/dist/semver/doc/semver.html) provides a semantic version data type. | 0.41.0 | |
...
| [temporal_tables](https://github.com/arkhipov/temporal_tables) supports the system-period temporal tables. | 1.2.2 | |
| [timescaledb](https://github.com/timescale/timescaledb) enables scalable inserts and complex queries for time-series data. | 2.24.0 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
...
| [vector](https://github.com/pgvector/pgvector) is a vector data type and `ivfflat` and `hnsw` access methods. | 0.8.2 | |
| [wal2json](https://github.com/eulerto/wal2json) is an output plugin for logical decoding. It means that the plugin has access to tuples produced by INSERT and UPDATE. Also, UPDATE/DELETE old row versions can be accessed depending on the configured replica identity. | 2.6 <sup>*</sup> | <sup>*</sup> Set the `shared_preload_libraries` parameter to enable the corresponding libraries. |
